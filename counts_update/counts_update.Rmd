...
xich <- sqlQuery(channel, "
  select a.tag_id, a.ich_obs, a.gra_obs, a.srrt,
    a.river_km, a.tag_site, a.capture_di,
  'mig_his' = case
...
pitflow2<- merge(xich, flow_dat, by= 'obs_date') # bring in flow data
pitflow2$ftt<- with(pitflow2, gra_obs-ich_obs)
pitflow2$ftt<- as.numeric(pitflow2$ftt)
pitflow2<- subset(pitflow2, (ftt>0 & ftt<100)) # get rid of is.na(ftt)
pitflow2<- pitflow2[substr(pitflow2$river_km, 1, 3)==522 & 
    substr(pitflow2$river_km, 5, 7)>=173,] # get rid of fish below grinite
...
# exclude trapy tag sites
pitflow2<- subset(pitflow2,
  !tag_site %in% c('CLWTRP','GRNTRP','IMNTRP','SNKTRP'))

# bring in temper data
td_temp<- td[,c(1,3)]
